...
export const JoinRoomScreen = ({ navigation }: any) => {
  ...
  const handleJoinByCode = async () => {
    ...
    setJoining(true);
    try {
      const res = await matchAPI.joinByCode(code);
      if (res.data.success) {
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        {/* Join by Code banner */}
        <TouchableOpacity
          style={styles.joinCodeBanner}
          onPress={() => setJoinCodeModal(true)}
          activeOpacity={0.7}
        >
          <View style={styles.joinCodeIcon}>
            <MaterialIcons name="vpn-key" size={18} color="#f47b25" />
          </View>
          <View style={{ flex: 1 }}>
            <Text style={styles.joinCodeTitle}>Have an invite code?</Text>
            <Text style={styles.joinCodeSubtitle}>Tap to join with a 6-character code</Text>
          </View>
