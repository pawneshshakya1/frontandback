  ...
  const verifyPayment = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        setPopup({ visible: true, type: "success", title: "Payment Successful!", message: `Successfully added ₹${amount} to your wallet!` });
      } else {
        setPopup({ visible: true, type: "error", title: "Payment Failed", message: "Transaction could not be verified." });
      }
    } catch (error: any) {
      setPopup({ visible: true, type: "error", title: "Error", message: error.response?.data?.message || "Verification failed" });
    } finally {
      setLoading(false);
          ...
          {paymentSessionId && (
            <WebView
              ...
              onShouldStartLoadWithRequest={(request) => {
                // Block non-Cashfree URLs (return URL, about:blank, etc.)
