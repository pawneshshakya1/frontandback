...
export const WithdrawScreen = ({ navigation }: any) => {
  ...
  const {
    ...
    fetchLastDepositSource,
    withdraw,
    isLoading: loading,
  } = useWalletStore();

  const maxWithdrawable = withdrawableBalance || 0;
  ...
  const fetchLastSource = async () => {
    try {
      const res = await fetchLastDepositSource();
      if (res.success && res.data) {
        setLastSource(res.data);
      }
    } catch (err) {
    ...
    <View style={styles.container}>
      ...
      <View style={[styles.bottomBtnContainer, { paddingBottom: insets.bottom + 16 }]}>
        <TouchableOpacity
          ...
          disabled={loading}
        >
          <Text style={styles.confirmBtnText}>{loading ? "PROCESSING..." : "CONFIRM WITHDRAWAL"}</Text>
