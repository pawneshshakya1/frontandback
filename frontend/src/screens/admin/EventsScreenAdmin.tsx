...
export const EventsScreenAdmin = ({ navigation }: any) => {
  ...
  const [popup, setPopup] = useState({ visible: false, title: '', message: '', type: 'error' as 'success' | 'error' | 'warning' | 'info' });

  const fetchMatches = async () => {
    try {
      setIsLoading(true);
      // Admin should only see matches they created or all matches? 
      // User request: "event created list by admin" -> Implies strictly matches created by THIS admin? 
      // Or "List of events created by ANY admin"?
      // Usually "My Events" is appropriate.
      // I'll assume we want to view ALL matches for now but with admin capabilities, OR specific admin matches.
      // Let's assume we want to list ALL matches so the admin can manage them.
      // IF the user specifically wants "My Created Events", we might need a filter.
      // Re-reading: "event created list by admin" -> "List of events created by admin"
      // If this screen is "EventsScreen", maybe it should show ALL events.
      // But if the user wants to see what *they* created, that's different.
      // Let's stick to showing ALL matches for now as an "Admin Dashboard" view, but maybe highlight created ones?
      // Actually, let's keep it as fetching all matches but rename header to "Manage Events".
      ...
      const response = await adminAPI.getMatches(params);
  ...
  const toggleNearby = async (value: boolean) => {
    if (value) {
      ...
      if (status !== 'granted') {
        setPopup({ visible: true, title: 'Permission denied', message: 'Allow location access to find nearby events.', type: 'warning' });
        return;
      }
  ...
  const getFilteredMatches = () => {
    ...
    switch (activeTab) {
      ...
      case "Upcoming":
        ...
        break;
      case "Results":
        results = results.filter(m => m.status === "COMPLETED" || m.status === "REVIEW");
        break;
    }
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={[styles.header, { paddingTop: insets.top + 12 }]}>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
          <View style={{
            ...
            <Text style={{ color: isNearby ? COLORS.primary : `${COLORS.textMuted}66`, fontSize: 10, fontWeight: 'bold' }}>NEARBY</Text>
            <Switch
              value={isNearby}
              onValueChange={toggleNearby}
              trackColor={{ false: `${COLORS.textMuted}33`, true: `${COLORS.primary}4D` }}
              thumbColor={isNearby ? COLORS.primary : '#f4f3f4'}
          ...
          >
            <MaterialIcons name="tune" size={20} color={activeFilters.gameType !== 'All' || activeFilters.mode !== 'All' || activeFilters.entryType !== 'All' ? COLORS.primary : "white"} />
          </TouchableOpacity>
        </View>
      </View>
      ...
      >
        {isLoading ? (
          <LoadingOverlay visible={true} message="Loading events..." />
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                ...
                <View style={styles.matchRowStats}>
                  ...
                  <View style={styles.rowRating}>
                    <MaterialIcons name="star" size={10} color={COLORS.eliteSupreme} />
              ...
              <View
                style={[
                  ...
                  {
                    backgroundColor:
                      item.status === "OPEN" ? COLORS.primary : COLORS.border,
                  },
                ]}
              >
                <Text
          ...
          <View style={styles.emptyContainer}>
            <MaterialIcons name="event-busy" size={48} color={`${COLORS.textMuted}1A`} />
        ...
        {activeTab === "Results" && filteredEvents.length === 0 && (
          <View style={styles.emptyContainer}>
            <MaterialIcons name="history" size={48} color={`${COLORS.textMuted}1A`} />
