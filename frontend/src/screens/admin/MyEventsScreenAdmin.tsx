...
export const MyEventsScreenAdmin = ({ navigation }: any) => {
    ...
    showConfirm(
      ...
      async () => {
        try {
          ...
          showSuccess("Event Live", "Event is now published and visible to all users!");
          fetchMatches();
        } catch (error: any) {
  ...
  const confirmCancel = async () => {
    ...
    try {
      const res = await matchAPI.cancelEvent(cancelModal.matchId, {
        reason: cancelModal.reason || "Cancelled by admin",
      });
      setCancelModal((prev) => ({
        ...prev,
        visible: false,
  ...
  const confirmSetCreds = async () => {
    ...
    try {
      await adminAPI.updateRoomDetails(credsModal.matchId, {
