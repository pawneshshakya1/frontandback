...
export const MatchDetailScreenPartner = ({ route, navigation }: any) => {
  ...
  const handleUpdateRoomConfirm = async () => {
    if (!updateRoomModal.roomId.trim()) {
      Alert.alert("Room ID Required", "Please enter the in-game Room ID.");
      return;
    }
    setUpdateRoomModal(prev => ({ ...prev, loading: true }));
    try {
      const payload: any = {
        room_id: updateRoomModal.roomId.trim(),
      };
      if (updateRoomModal.roomPassword.trim()) {
        payload.room_password = updateRoomModal.roomPassword.trim();
      }
      await api.put('/matches/' + match._id, payload);
      setUpdateRoomModal(prev => ({ ...prev, visible: false, loading: false }));
      Alert.alert("Success", "Room details updated!");
      fetchMatch();
    } catch (e: any) {
      setUpdateRoomModal(prev => ({ ...prev, loading: false }));
    ...
    Alert.alert("Delete Match", "Are you sure you want to delete this draft?", [
      { text: "Cancel", style: "cancel" },
      {
        text: "Delete", style: "destructive", onPress: async () => {
          try {
            await api.delete('/matches/' + match._id, { params: { force: "true" } });
            navigation.goBack();
  ...
  const handlePublish = async () => {
    try {
      await api.post('/partner/events/' + match._id + '/publish');
  ...
  return (
    <View style={styles.container}>
            ...
            <View style={styles.statusRow}>
              ...
              {match.room_password && (
                <TouchableOpacity onPress={() => setShowPassword(!showPassword)} style={styles.eyeBtn}>
                  <MaterialIcons name={showPassword ? "visibility-off" : "visibility"} size={18} color="rgba(255,255,255,0.5)" />
                </TouchableOpacity>
              )}
            </View>
            {match.room_password && (
          ...
          <View style={styles.card}>
            ...
            {match.room_password && (
              <>
                <View style={styles.divider} />
                <View style={styles.ruleItem}>
                  <MaterialIcons name="lock" size={16} color={COLORS.accentBlue} />
                  ...
                  <TouchableOpacity onPress={() => setShowPassword(!showPassword)} style={{ marginLeft: 4 }}>
                    <MaterialIcons name={showPassword ? "visibility-off" : "visibility"} size={16} color="rgba(255,255,255,0.5)" />
                  </TouchableOpacity>
                </View>
              </>
            )}
      ...
      <BlurView intensity={20} tint="dark" style={styles.bottomBar}>
        ...
        ) : isCreator && !isDraft && isMatchUpcoming(match) ? (
          <TouchableOpacity
            style={[styles.joinBtn, { flex: 1 }]}
      ...
      {/* Update Room Modal */}
      <Modal
        visible={updateRoomModal.visible}
        transparent
        animationType="fade"
        onRequestClose={() => setUpdateRoomModal(prev => ({ ...prev, visible: false }))}
      >
        <KeyboardAvoidingView
        ...
        >
          <View style={styles.qrCard}>
            <View style={styles.qrHeader}>
              <Text style={styles.qrTitle}>Update Room Details</Text>
              <TouchableOpacity onPress={() => setUpdateRoomModal(prev => ({ ...prev, visible: false }))}>
                <MaterialIcons name="close" size={24} color="white" />
              </TouchableOpacity>
            </View>

            <Text style={[styles.modalHint, { color: '#f47b25', marginBottom: 16 }]}>
            ...
            <TextInput
              ...
              value={updateRoomModal.roomId}
              onChangeText={v => setUpdateRoomModal(prev => ({ ...prev, roomId: v }))}
            />

            <Text style={styles.infoLabel}>ROOM PASSWORD <Text style={{ color: 'rgba(255,255,255,0.3)', fontWeight: 'normal' }}>(optional)</Text></Text>
            <TextInput
              style={styles.modalInput}
              placeholder="Enter Room Password"
              placeholderTextColor="rgba(255,255,255,0.25)"
              value={updateRoomModal.roomPassword}
              onChangeText={v => setUpdateRoomModal(prev => ({ ...prev, roomPassword: v }))}
              secureTextEntry
            />
            ...
            <View style={{ flexDirection: 'row', gap: 12, marginTop: 8 }}>
              <TouchableOpacity
                ...
                onPress={() => setUpdateRoomModal(prev => ({ ...prev, visible: false }))}
                disabled={updateRoomModal.loading}
              >
                <Text style={{ color: 'rgba(255,255,255,0.6)', fontSize: 14, fontWeight: 'bold', textAlign: 'center' }}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[styles.joinBtn, { flex: 2 }]}
                onPress={handleUpdateRoomConfirm}
                disabled={updateRoomModal.loading}
              >
                {updateRoomModal.loading ? (
                  ...
                  >
                    <Text style={styles.joinBtnText}>SAVE</Text>
                  </LinearGradient>
                )}
              </TouchableOpacity>
            </View>
