...
import { partnerAPI, matchAPI } from "../../services/api";
...
export const MyEventsScreenPartner = ({ navigation }: any) => {
  ...
  const fetchMatches = async () => {
    try {
      const res = await partnerAPI.getEvents();
  ...
  const handlePublish = (id: string, title: string) => {
    showConfirm(
      ...
      async () => {
        try {
          await partnerAPI.publishEvent(id);
  ...
  const confirmSetCreds = async () => {
    ...
    try {
      await partnerAPI.updateRoomDetails(credsModal.matchId, {
