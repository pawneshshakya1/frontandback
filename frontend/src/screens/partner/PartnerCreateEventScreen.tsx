...
export const PartnerCreateEventScreen = ({ navigation }: any) => {
    ...
    const validateForm = () => {
        ...
        if (entryFee > getEntryFeeLimit()) {
            showPopup("warning", "Entry Fee Limit", `Entry fee ₹${entryFee} exceeds your tier limit of ₹${getEntryFeeLimit()}.`, () => navigation.navigate("PartnerTier"));
            return false;
        }

        const prizePool = parseFloat(formData.prize_pool) || 0;
        if (prizePool > getPrizePoolLimit()) {
            showPopup("warning", "Prize Pool Limit", `Prize pool ₹${prizePool} exceeds your tier limit of ₹${getPrizePoolLimit().toLocaleString()}.`, () => navigation.navigate("PartnerTier"));
            return false;
        }

        const maxPlayers = parseInt(formData.max_players) || 48;
        if (maxPlayers > getMaxPlayersLimit()) {
            showPopup("warning", "Player Limit", `Max players ${maxPlayers} exceeds your tier limit of ${getMaxPlayersLimit()}.`, () => navigation.navigate("PartnerTier"));
            return false;
        }
    ...
    const eventTypes = [
        { key: "STANDARD" as const, label: "Standard", icon: "event", allowed: true },
        { key: "SPONSORED" as const, label: "Sponsored", icon: "campaign", allowed: tierInfo?.benefits?.can_create_sponsored ?? false },
        { key: "PREMIUM" as const, label: "Premium", icon: "workspace-premium", allowed: tierInfo?.benefits?.can_create_premium ?? false },
    ];
            ...
            <View style={[styles.header, { marginTop: insets.top + 10 }]}>
                ...
                <TouchableOpacity onPress={() => navigation.navigate("PartnerTier")}>
                    <LinearGradient colors={[currentTierColor + "40", currentTierColor + "20"]} style={styles.tierBadge}>
                        <MaterialIcons
                            name={tierInfo?.current_tier === "premium" ? "workspace-premium" : tierInfo?.current_tier === "sponsored" ? "campaign" : "shield"}
                            size={12}
                    ...
                    {eventTypes.map((type) => {
                        ...
                        return (
                            <TouchableOpacity
                                ...
                                onPress={() => {
                                    if (isDisabled) {
                                        ...
                                        showPopup("warning", `${requiredTier} Tier Required`, `You need ${requiredTier} tier to create ${type.label} events.`, () => navigation.navigate("PartnerTier"));
                                        return;
                                    }
                            ...
                            >
                                ...
                                />
                                <Text style={[styles.typeText, eventType === type.key && styles.typeTextActive, isDisabled && styles.typeTextDisabled]}>
                                    {type.label}
                                </Text>
                                {!type.allowed && (
