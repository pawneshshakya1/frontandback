...
export const PersonalInformationScreenPartner = ({ navigation }: any) => {
  ...
  const fetchProfile = async () => {
    try {
      ...
      if (partnerRes.data.success && userRes.data.success) {
        const partnerProfile = partnerRes.data.data;
        const userObj = userRes.data.data?.user || {};
        const userProfile = userRes.data.data?.profile || {};
        ...
        setFormData({
          ...
          phone: partnerProfile.phone || userProfile.phone || "",
          gender: userProfile.gender || "Male",
          bio: partnerProfile.bio || userProfile.bio || "",
          gameUidName: userProfile.game_uid_name || "",
          ffMaxUid: userProfile.ff_max_uid || "",
          guildName: userProfile.guild_name || "",
          guildUid: userProfile.guild_uid || "",
          preferredRole: userProfile.preferred_role || "",
          discordTag: userProfile.discord_tag || "",
          instagram: userProfile.instagram || "",
          facebook: userProfile.facebook || "",
          x: userProfile.x_twitter || "",
          threads: userProfile.threads || "",
          youtube: userProfile.youtube || "",
          discordServer: userProfile.discord_server || "",
        });

        setAvatar(userProfile.avatar || partnerProfile.logo_url || null);
        setBackgroundImage(userProfile.background_image || partnerProfile.banner_url || null);
        setIsEmailVerified(userObj.email === authData?.email);
      }
    } catch (error) {
      console.error("Failed to fetch partner/user profile:", error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    try {
      ...
      const [partnerResponse, userResponse] = await Promise.all([
        api.put('/partner/profile', partnerPayload),
        api.put('users/profile', userPayload)
      ]);

      if (partnerResponse.data.success && userResponse.data.success) {
        ...
        showError("Error", "Failed to update one or more profiles");
      }
    } catch (error: any) {
        ...
        <View style={{ marginTop: 20 }}>
          <View style={styles.form}>
            ...
            <InputField
              ...
              onChangeText={(t: string) => setFormData({...formData, businessName: t})}
            />
            ...
            <InputField
              ...
              placeholder="Tell hosts and players about your business"
            />
